...
def mouse_callback(event, x, y, flags, param):
    ...
    if event == cv.EVENT_LBUTTONDOWN:
        ...
        if hsv[0] < 10:
            ...
            upper_blue1 = np.array([180, 225, 225])
            lower_blue2 = np.array([0, threshold, threshold])
            upper_blue2 = np.array([hsv[0], 225, 225])
            lower_blue3 = np.array([hsv[0], threshold, threshold])
            upper_blue3 = np.array([hsv[0] + 10, 225, 225])
            ...
            upper_blue1 = np.array([180, 225, 225])
            lower_blue2 = np.array([0, threshold, threshold])
            upper_blue2 = np.array([hsv[0] + 10 - 180, 225, 225])
            lower_blue3 = np.array([hsv[0] - 10, threshold, threshold])
            upper_blue3 = np.array([hsv[0], 225, 225])
            #     print(i, 180, 0, i+10-180)
            #     print(i-10, i)
        else:
            print("case3")
            lower_blue1 = np.array([hsv[0], threshold, threshold])
            upper_blue1 = np.array([hsv[0] + 10, 225, 225])
...
while True:
    ...
    img_mask_d1 = cv.bitwise_or(img_mask1, img_mask2)
    img_mask_d2 = cv.bitwise_or(img_mask_d1, img_mask3)

    kernel = np.ones((11, 11), np.uint8)
    img_mask = cv.morphologyEx(img_mask_d2, cv.MORPH_OPEN, kernel)
